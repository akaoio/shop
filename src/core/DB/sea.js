;(function () {
    ...
    USE(function (module) {
        ...
        try {
            if (SEA.window) {
                if (location.protocol.indexOf("s") < 0 && location.host.indexOf("localhost") < 0 && !/^127\.\d+\.\d+\.\d+$/.test(location.hostname) && location.protocol.indexOf("blob:") < 0 && location.protocol.indexOf("file:") < 0 && location.origin != "null") {
    ...
    USE(function (module) {
        ...
        function pointAdd(p1, p2) {
            ...
            let lambda = p1.x === p2.x && p1.y === p2.y ? mod((3n * mod(p1.x ** 2n, P) + A) * modInv(2n * p1.y, P), P) : mod(mod(p2.y - p1.y, P) * modInv(mod(p2.x - p1.x, P), P), P)
        ...
        SEA.pair =
            ...
            (async (cb, opt) => {
                try {
                    ...
                    const biToB64 = (n) => shim.Buffer.from(n.toString(16).padStart(64, "0"), "hex").toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "")
                    ...
                    const seedToKey = async (seed, salt) => {
                        ...
                        const buf = typeof seed === "string" ? enc.encode(seed).buffer : seed instanceof ArrayBuffer ? seed : seed && seed.byteLength !== undefined ? seed.buffer || seed : null
                    ...
                    if (opt.priv) {
                        ...
                        if (opt.epriv) {
                            ...
                            try {
                                const dh = await ecdhSubtle.generateKey({ name: "ECDH", namedCurve: "P-256" }, true, ["deriveKey"]).then(async (k) => ({
                                    epriv: (await ecdhSubtle.exportKey("jwk", k.privateKey)).d,
                                    epub: (await ecdhSubtle.exportKey("jwk", k.publicKey)).x + "." + (await ecdhSubtle.exportKey("jwk", k.publicKey)).y
                                }))
                                r.epriv = dh.epriv
                                r.epub = dh.epub
                            } catch (e) {}
                        ...
                        if (opt.priv) {
                            ...
                            const sa = await subtle.generateKey({ name: "ECDSA", namedCurve: "P-256" }, true, ["sign", "verify"]).then(async (k) => ({
                                priv: (await subtle.exportKey("jwk", k.privateKey)).d,
                                pub: (await subtle.exportKey("jwk", k.publicKey)).x + "." + (await subtle.exportKey("jwk", k.publicKey)).y
                            }))
                            r.priv = sa.priv
                            r.pub = sa.pub
                        }
                        ...
                        r = {
                            priv: biToB64(signPriv),
                            pub: pubFromPriv(signPriv),
                            epriv: biToB64(encPriv),
                            epub: pubFromPriv(encPriv)
                        }
                    } else {
                        const sa = await subtle.generateKey({ name: "ECDSA", namedCurve: "P-256" }, true, ["sign", "verify"]).then(async (k) => ({
                            priv: (await subtle.exportKey("jwk", k.privateKey)).d,
    ...
    USE(function (module) {
        ...
        async function k(p, j, o, c) {
            ...
            var h = await sha(j)
            var s = await (shim.ossl || shim.subtle)
                .importKey("jwk", x, S.ecdsa.pair, false, ["sign"])
                .then((k) => (shim.ossl || shim.subtle).sign(S.ecdsa.sign, k, new Uint8Array(h)))
                .catch(() => {
                    throw "SEA signature failed"
                })
        ...
        SEA.sign =
            ...
            (async (data, pair, cb, opt) => {
                try {
                    ...
                    if (SEA.verify && (S.check(c) || (c && c.s && c.m)) && u !== (await SEA.verify(c, pair))) {
    ...
    USE(function (module) {
        ...
        async function w(j, k, s) {
            ...
            var e = btoa(String.fromCharCode(...new Uint8Array(m)))
                .replace(/\+/g, "-")
                .replace(/\//g, "_")
                .replace(/=/g, "")
            if (JSON.parse(c).challenge !== e) throw "Challenge verification failed"
            var h = await (shim.ossl || shim.subtle).digest({ name: "SHA-256" }, new TextEncoder().encode(c))
        ...
        async function v(j, k, s, h) {
            return (shim.ossl || shim.subtle).verify({ name: "ECDSA", hash: { name: "SHA-256" } }, k, s, new Uint8Array(h))
        }
        ...
        SEA.verify =
            ...
            (async (d, p, cb, o) => {
                try {
                    ...
                    try {
                        var k = await (shim.ossl || shim.subtle).importKey(
                            "jwk",
                            {
                                kty: "EC",
                                crv: "P-256",
        ...
        SEA.opt.fall_verify = async function (data, pair, cb, opt, f) {
            ...
            var json = await S.parse(data),
                key = await SEA.opt.slow_leak(pair.pub || pair)
            var hash = !f || f <= SEA.opt.fallback ? shim.Buffer.from(await shim.subtle.digest({ name: "SHA-256" }, new shim.TextEncoder().encode(await S.parse(json.m)))) : await sha(json.m)

            try {
                var buf = shim.Buffer.from(json.s, opt.encode || "base64")
                var sig = new Uint8Array(buf)
                var check = await (shim.ossl || shim.subtle).verify({ name: "ECDSA", hash: { name: "SHA-256" } }, key, sig, new Uint8Array(hash))
                if (!check) throw ""
            } catch (e) {
                try {
            ...
            SEA.opt.fall_soul = tmp["#"]
            SEA.opt.fall_key = tmp["."]
            SEA.opt.fall_val = data
            SEA.opt.fall_state = tmp[">"]
            if (cb) {
                try {
                    cb(r)
                } catch (e) {
    ...
    USE(function (module) {
        var u,
            Gun = "" + u != typeof GUN ? GUN || { chain: {} } : USE(("" + u === typeof MODULE ? "." : "") + "./gun", 1)
